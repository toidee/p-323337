...
const InspectorCalendar = () => {
  ...
  // Filter events for the current inspector
  const inspectorEvents = events.filter(
    event => event.inspector_id === user?.id || user?.role === "admin"
  );

  // Filter events by status if a status is selected
  const filteredEvents = selectedStatus === "all"
    ? inspectorEvents
    : inspectorEvents.filter(event => event.status === selectedStatus);
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              View your scheduled inspections.
            </p>
          </div>
